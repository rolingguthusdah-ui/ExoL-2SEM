...
(function() {
    ...
    app.carousel = {
        ...
        updateBackground: function() {
            const selectedImage = $(".selected img");

            if (selectedImage) {
                document.body.style.setProperty("--carousel-bg", `url("${selectedImage.src}")`);
            }
        },
        move: function(el) {
            let selected = el;
...
// Select all carousel item containers
const albumItems = document.querySelectorAll('#carousel > div');
...
albumItems.forEach(item => {
    item.addEventListener('click', () => {
        ...
        if (audioSrc) {
            // Stop and reset previous audio if one is already playing
            if (currentAudio) {
                currentAudio.pause();
                currentAudio.currentTime = 0;
            }

            // Create and play the new audio track
...
// Function to update the body background image
function updateBackground() {
    const selectedItem = document.querySelector('#carousel .selected');
    if (selectedItem) {
        const bgImage = selectedItem.getAttribute('data-bg');
        if (bgImage) {
...
// Run once on page load to set the initial background image
window.addEventListener('DOMContentLoaded', updateBackground);

// --- Your Existing Carousel Click Handling ---
// Inside your code where the "Next" and "Previous" buttons move classes,
// make sure to call updateBackground() at the very end of those changes.
// Example:
document.getElementById('next').addEventListener('click', () => {
    // ... your existing code that rotates classes (selected, prev, next) ...
    
    updateBackground(); // Call this right after you update the classes
});
